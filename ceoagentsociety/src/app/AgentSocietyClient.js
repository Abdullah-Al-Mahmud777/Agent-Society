...
const agentRoster = [
	...
	},
	{
		...
		role: "Go-to-market",
		summary: "Defines positioning, launch channels, and messaging.",
...
function buildCouncilResponse(input) {
	...
	const isAiStartup = normalized.includes("ai startup") || normalized.includes("build an ai startup");
...
export default function AgentSocietyClient() {
	...
	const [submittedIdea, setSubmittedIdea] = useState("I want to build an AI startup");

	const council = useMemo(() => buildCouncilResponse(submittedIdea), [submittedIdea]);

	const handleRunCouncil = () => {
		const trimmed = idea.trim();
		setSubmittedIdea(trimmed.length ? trimmed : "I want to build an AI startup");
	};
					...
					<div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
						<div className="max-w-3xl">
							...
							<h1 className="mt-5 text-4xl font-semibold tracking-tight text-white sm:text-5xl lg:text-6xl">
								A simple agent system for startup planning.
							</h1>
					...
					<SectionCard title="Try the council">
						...
						<div className="mt-5 space-y-3">
							...
							<textarea
								...
								placeholder="Example: I want to build an AI startup"
							/>
						</div>
